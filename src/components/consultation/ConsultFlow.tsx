...
export default function ConsultFlow() {
  ...
  const routeSuggestion = () => {
    if (answers.need === "A business system")
      return { label: "Business systems", href: "/services/business-systems" };
    if (answers.need === "AI or automation")
      return { label: "AI & automation services", href: "/services/ai" };
    if (answers.need === "A software system")
      return { label: "Software systems", href: "/services/software" };
    if (answers.need === "A website")
      return { label: "Web design & development", href: "/services/websites" };
  ...
  const mailtoLink = () => {
    const subject = encodeURIComponent(
      `New project inquiry — ${answers.need || ""}`,
    );
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex gap-[6px] mb-[22px]">
        {STEPS.map((_, i) => (
          <span
            key={i}
            className={`h-[2px] flex-1 ${i <= Math.min(stepIndex, STEPS.length - 1) ? "bg-[var(--ink)]" : "bg-[var(--line-strong)]"}`}
          />
        ))}
        <span
          className={`h-[2px] flex-1 ${isComplete ? "bg-[var(--ink)]" : "bg-[var(--line-strong)]"}`}
        />
      </div>
        ...
        <AnimatePresence mode="wait">
          {!isComplete ? (
            ...
            >
              <div
                id="question-label"
                className="text-[18px] sm:text-[22px] font-display font-bold mb-[18px] leading-[1.3]"
              >
                {STEPS[stepIndex].q}
              </div>
              <div
            ...
            <motion.div
              ...
              transition={{ duration: 0.25, ease: [0.65, 0, 0.35, 1] }}
            >
              <div className="text-[22px] font-display font-bold mb-[18px] leading-[1.3]">
              ...
              <div className="text-[14px] leading-[1.8] text-[var(--ink-soft)]">
                {STEPS.map((s) => (
                  <div
                    key={s.key}
                    className="flex justify-between gap-[10px] py-[10px] border-b border-[var(--line)]"
                  >
                    <span>{s.q.replace("?", "")}</span>
                    <strong className="text-[var(--ink)] font-semibold text-right">
                      {answers[s.key] || "—"}
                    </strong>
                  </div>
                ))}
              ...
              <div className="mt-[26px] flex flex-col gap-[12px]">
                ...
                <div className="flex justify-center mt-[4px]">
                  <LinkArrow href={suggestion.href}>
                    Or browse {suggestion.label} →
                  </LinkArrow>
                </div>
              </div>
            </motion.div>
