...
class Particle {
  ...
  constructor({ x, y, pinned, col, row, char }: any) {
    ...
    this.pinned = pinned;
    this.homeX = x;
    this.col = col; this.row = row; this.char = char;
  }
  update(delta: number, gravity: number, damping: number, windAmp: number, windFreq: number, t: number) {
    ...
    const wind = Math.sin(t * windFreq + this.col * 0.14 + this.row * 0.05) * windAmp;
    this.acceleration.x += wind / dd;
    this.acceleration.y += gravity / dd;
...
export function mountKineticWord(container: HTMLElement, opts?: any) {
  const cfg = Object.assign({
    ...
    windAmp: 0.028,
    windFreq: 0.0016,
    compressV: 0.35, stretchV: 1.55,
    compressH: 0.55, stretchH: 1.9,
    mouseRadius: 2600,
    mouseStrength: 2.2,
    interactive: true,
    grabRadius: 16
  ...
  let canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, particles: Map<string, Particle>, constraints: Constraint[], input: Input, rafId: number, resizeObs: any;
  let widthPx = 0, heightPx = 0, running = true;
  ...
  function measureAndBuildMask(stageW: number, stageH: number) {
    ...
    const maskW = cols * spacing, maskH = rows * spacing;
  ...
  function build() {
    ...
    grid.forEach((pos: any, key: any) => {
      ...
      const ch = charList[i % charList.length]; i++;
      particles.set(key, new Particle({
        x: pos.x + offsetX, y: pos.y + offsetY, pinned, col, row, char: ch
      }));
    });
    ...
    particles.forEach((p, key) => {
      ...
      if (particles.has(rightKey)) {
        constraints.push(new Constraint({ p1: p, p2: particles.get(rightKey), length: spacing, compress: cfg.compressH, stretch: cfg.stretchH }));
      }
    ...
    function draw() {
      ...
      particleList.forEach(p => {
        ...
        const tx = p.pos.x + ox, ty = p.pos.y + oy;
    ...
    function loop(t: number) {
      ...
      particleList.forEach(p => p.update(delta ? 16.6 : 16.6, cfg.gravity, cfg.damping, cfg.windAmp, cfg.windFreq, t));
      lastDelta = t;
  ...
  class Input {
    ...
    constructor({ canvas, particles, cfg }: any) {
      this.canvas = canvas; this.particles = particles; this.cfg = cfg;
      this.mouse = new Vec2();
      this.bind();
    }
    setMouse(e: any) {
    ...
    up() {
      if (this.grabbed) { this.grabbed.pinned = this.grabbed._wasPinned!; this.grabbed = null; }
    }
    move(e: any) {
      this.setMouse(e);
      if (this.grabbed) {
    ...
    bind() {
      this.down = this.down.bind(this); this.up = this.up.bind(this); this.move = this.move.bind(this);
