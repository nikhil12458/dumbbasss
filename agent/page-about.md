# About Page

| Field | Value |
|-------|-------|
| **Path** | `src/app/about/page.tsx` |
| **Author** | 🤖 AI |
| **Category** | Page |

## Purpose

The studio's identity page — explains the name, the philosophy, and the fit.

## Sections

1. **Hero** — "The name is a joke. The work is *not*."
2. **Kinetic Word** — A smaller instance of the KineticWord canvas as a visual accent
3. **Why the name** — 2-column editorial layout (sticky sidebar label + prose)
4. **Who this is for** — Same 2-column layout
5. **Good fit / Not a great fit** — Side-by-side numbered lists
6. **How we think** — "A studio of one, built like a team of ten." + 4-row principle grid (Design before code, Staging link always, Plain-language calls, Small scope shipped properly)
