...
The hero interactive element of the site — the word "dumbbasss" rendered as a grid of characters that hang like cloth and react to mouse movement, gravity, and wind.

## Architecture

### `kinetic-type.ts` (utility)
A self-contained physics engine ported from the original vanilla JS:

- **`Vec2`** — 2D vector math
- **`Particle`** — A single character position with Verlet integration, gravity, wind, and damping
...
| `windAmp` | `number` | `0.028` | Wind turbulence amplitude |
| `gravity` | `number` | `0.05` | Downward pull strength |
